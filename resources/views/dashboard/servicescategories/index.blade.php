        ...
        <section class="content">
            ...
            <div class="box box-primary">
                ...
                <div class="box-header with-border">
                    ...
                    <form action="{{ route('dashboard.catogeries.index') }}" method="get">

                        <div class="row">

                            <div class="col-md-4">
                                @if (auth()->user()->hasPermission('create_categories'))
                                    <a href="{{ route('dashboard.catogeries.create') }}" class="btn btn-primary"><i
                ...
                <div class="box-body table-responsive no-padding on-overflow-x minHeight">
                    ...
                    @if ($catogeries->count() > 0)
                        <table class="table table-hover" id="table">
                            ...
                            <tbody>
                            @foreach ($catogeries as $index=>$type)
                                <tr>
                                    <td>{{ $index + 1 }}</td>
                                        ...
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                @if (auth()->user()->hasPermission('update_categories'))
                                                    <a href="{{ route('dashboard.catogeries.edit', $type->id) }}"
                                                ...
                                                <br>
                                                @if (auth()->user()->hasPermission('update_categories'))
                                                    <form action="{{ route('dashboard.catogeries.block', $type->id) }}"
                                                ...
                                                @if (auth()->user()->hasPermission('delete_categories'))
                                                    <form
                                                        action="{{ route('dashboard.catogeries.destroy', $type->id) }}"
                                                        method="post" style="display: inline-block">
                                                        {{ csrf_field() }}
                                                        {{ method_field('delete') }}
